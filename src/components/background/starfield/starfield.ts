...
export class StaticStarfield {
	...
	targetOfStar(star: Star) {
		...
		let vector3 = vec2add(
			currentNode,
			vec2mul(vec2sub(nextNode, currentNode), vec2(star.NodePercent)),
		);
	...
	static ScreenPosOfStar(star: Star, dim: Vec2 = vec2(448, 212)) {
		const vec2mod = (a: Vec2, b: Vec2) => vec2(a.x % b.x, a.y % b.y);
		const mod = (x: Vec2, m: Vec2) => vec2mod(vec2add(vec2mod(x, m), m), m);

		return vec2sub(vec2add(vec2(-64, -16), mod(star.position, dim)), 16);
	}
