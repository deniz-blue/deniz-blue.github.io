...
class DEVICE_OBACK_4 {
	...
	step() {
		...
		if (this.OBSPEED > 0) this.alpha = Math.sin(this.siner / 34) * 0.2;

		this.ystretch += this.OBSPEED;
		this.xstretch += this.OBSPEED;

		if (this.b_insurance < 0) this.b_insurance += 0.01;

		if (this.ystretch > 2) {
			this.o_insurance += 0.01;

			if (this.o_insurance >= 0.5) this.destroy();
		}
	}
...
export const DepthBackground = () => {
		...
		return {
			...
			draw() {
				...
				for (let dv of instances) {
					...
					ctx.drawImage(sprite, 160 - dw / 2, 120 - dh / 2, dw, dh);
	...
	return (
		<div className="pageBackground">
			...
			<audio src={AUDIO_ANOTHERHIM} ref={audioRef} autoPlay loop />
		</div>
	);
};
