...
export const RefugeBackground = () => {
	const init: UseCanvasInit = useCallback((ctx) => {
		...
		const drawRect = (pos: Vec2, size: Vec2, c: string, outline?: string) => {
			ctx.fillStyle = c;
		...
		const drawGrid = (
			...
			for (let ix = 0; ix < amount.x; ix++) {
				for (let iy = 0; iy < amount.y; iy++) {
					drawRect(vec2add(pos, vec2mul(vec2add(size, spacing), vec2(ix, iy))), size, c, o);
				}
			}
		};
		...
		const buildingBase = ({
			...
			const buildingSize = vec2(
				padding.x +
					windowSize.x * windowAmount.x +
					windowSpacing.x * (windowAmount.x - 1) +
					padding.x,
				ctx.canvas.height - pos.y - 1,
			);

			drawRect(pos, buildingSize, color, outlineColor);
			drawGrid(vec2add(pos, padding), windowSize, windowAmount, windowSpacing, windowColor);
			...
			if (windowLitColor && windowLitIndicies?.length && windowLitSize) {
				for (let idx of windowLitIndicies) {
					...
					let center = vec2add(pos, padding, offset, vec2div(windowSize, 2));
